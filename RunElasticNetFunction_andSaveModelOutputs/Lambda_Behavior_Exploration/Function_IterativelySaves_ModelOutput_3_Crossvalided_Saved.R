Function_IterativelySaves_ModelOutput_3_Crossvalided_Saved <- function(data, Num_of_iteration, pathname){
  ...
  My_Sample <<- lapply(1:Num_of_iteration, function(i) sample(nrow(Experimental_Measures_final),
                                              replace = F))
    # 
  ## Modeling across 200 samples to generalize model's behavior
  alpha <- 0.5
  ...
  for (i in 1:Num_of_iteration){
    ...
    ElasticNetFunctions_InternalCode_2_crossvalidated_Saved(alphA = alpha, 
                                          partition_percentage = 1, Sample)
    # AllModels_Lambdas_Info <<- rbind(AllModels_Lambdas_Info, EachModel_lambda_info)
    
    Performance_training[i] <- (sum(Experimentaldata_withProb_Train$Ohio_Label == 
                                      Experimentaldata_withProb_Train$Exp_Prediction))/ nrow(Experimentaldata_withProb_Train)
    ...
    Coefficients[[i]] <- 
      as.matrix(coef(My_experimental_model_Final))[as.matrix(coef(My_experimental_model_Final))!=0]
  ...
  #                      file = paste(pathname, '/ModelsOutputTrain_CV_200Iterations.xlsx', sep = ""))
  # openxlsx::write.xlsx(FeatureandCoefficients,
  #                      file = paste(pathname, '/FeatureandCoefficients_CV_200Iterations.xlsx', sep = ""))
  # openxlsx::write.xlsx(AllModels_Lambdas_Info,
  #                      file = paste(pathname, '/AllModels_Lambdas_Info_CV_200Iterations.xlsx', sep = ""))}